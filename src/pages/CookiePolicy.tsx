  ...
  return (
    <>
      <SEO
        ...
        description="Informativa sull'uso dei cookie e delle tecnologie simili sul sito Il Bel Paese: cookie tecnici e di terze parti, durata e gestione."
      ...
      <article className="editorial-container py-12 prose prose-neutral max-w-3xl font-body">
        ...
        <p>I cookie sono piccoli file di testo che i siti visitati salvano nel dispositivo dell'utente per memorizzare informazioni utili al funzionamento del sito o per raccogliere dati statistici. Possono essere installati dal sito stesso (cookie di prima parte) o da domini terzi (cookie di terze parti). Tecnologie simili includono <em>localStorage</em> e <em>sessionStorage</em> del browser.</p>
        ...
        <ul>
          <li><strong>Sessione e autenticazione</strong> — token di sessione utilizzati dal backend Lovable Cloud per mantenere l'utente connesso all'area riservata. Salvati in <em>localStorage</em>. Durata: fino al logout o alla scadenza del refresh token.</li>
          <li><strong>Preferenze interfaccia</strong> — memorizzano impostazioni di visualizzazione e stato dei consensi. Durata: fino a 12 mesi.</li>
          <li><strong>Sicurezza</strong> — token anti-CSRF e protezione del modulo contatti. Durata: sessione.</li>
        </ul>

        <h3>2.2 Servizi di terze parti</h3>
        <ul>
          <li><strong>Lovable Cloud</strong> — fornitore dell'infrastruttura di autenticazione, database, storage ed edge functions. Imposta token tecnici necessari alla sessione utente. Server in area UE.</li>
          ...
          <li><strong>Photon (Komoot)</strong> — servizio di geocoding utilizzato per l'autocompletamento degli indirizzi. Effettua chiamate solo quando l'utente digita nel relativo campo.</li>
        </ul>
        <p>Non utilizziamo cookie di profilazione pubblicitaria, retargeting, analytics di terze parti né social plugin con tracciamento.</p>
        ...
        <p>La disattivazione dei cookie tecnici può compromettere alcune funzionalità del sito, in particolare l'accesso all'area riservata e l'invio del modulo contatti.</p>
